...
int main(int argc, char **argv)
{
    ...
    for (const auto &entry : std::filesystem::directory_iterator(path))
    {
        ...
        puts("\n\n");
        string turma = parseTurma(imgGray);

        size_t pos1 = turma.find(" ");
        string word1 = turma.substr(0, pos1);

        size_t pos2 = turma.find(" ", pos1 + 1);
        string word2 = turma.substr(pos1 + 1, pos2 - pos1 - 1);
        ...
        j[i][word1]["quarta"] = quartaH;
        j[i][word1]["quinta"] = quintaH;
        j[i][word1]["sexta"] = sextaH;
